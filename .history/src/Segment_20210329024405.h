...
namespace YSB
{
    ...
    template <class T>
    int intersect(const Segment<T, 2> &seg1, Line<T, 2> &l2, std::vector<Point<T, 2>> &result, Real tol = TOL)
    {
    ...
    template <class T>
    int intersect(const Segment<T, 2> &seg1, const Segment<T, 2> &seg2, std::vector<Point<T, 2>> &result, Real tol = TOL)
    {
        Point<T, 2> p1 = seg1[0], p2 = seg1[1],
                    p3 = seg2[0], p4 = seg2[1];
