...
namespace YSB
{
    ...
    struct TriangleIntersection
    {
        ...
        typedef typename std::vector<std::pair<std::vector<Segment<T, 3>>, std::vector<std::pair<int, int>>>> *Resultp;
        void operator()(
            const std::vector<Triangle<T, 3>> &inputA,
            const std::vector<Triangle<T, 3>> &inputB, Real tol = TOL, int w = 1);
        void work1(
            const std::vector<Triangle<T, 3>> &inputA,
    ...
    inline void TriangleIntersection<T>::operator()(
        const std::vector<Triangle<T, 3>> &inputA,
        const std::vector<Triangle<T, 3>> &inputB, Real tol, int w)
    {
        if (w == 1)
    ...
    template <class T>
    void TriangleIntersection<T>::assureworksame(
        Resultp *backup,
        Resultp *result)
    {
    ...
    template <class T>
    inline void TriangleIntersection<T>::work1(const std::vector<Triangle<T, 3>> &inputA, const std::vector<Triangle<T, 3>> &inputB, Real tol)
    {
    ...
    inline void TriangleIntersection<T>::work2(
        const std::vector<Triangle<T, 3>> &inputA,
        const std::vector<Triangle<T, 3>> &inputB, Real tol)
    {
        int numA = inputA.size(),
            numB = inputB.size();
        resultA.resize(numA);
        ...
        std::map<std::pair<int, int>, std::pair<Point<T, 1>, Point<T, 1>>>
            minmaxX, minmaxY;
        ...
        for (auto &&p : allP)
        {
            ...
            for (auto &&idTri2 : interTrisadd)
            {
                ...
                auto minmaxXid = TriX.minmax(pCmp),
                     minmaxYid = TriY.minmax(pCmp);
