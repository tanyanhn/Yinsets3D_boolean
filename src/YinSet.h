...
namespace YSB
{
    ...
    public:
        ...
        YinSet<T> meet(const YinSet<T> &y2, const int usingRFB = 1, Real tol = TOL) const;
        ...
        void objOutput(std::string s, std::string folder = "./", int prec = -std::log10(TOL))
        {
    ...
    template <class T>
    inline void YinSet<T>::collapse(std::vector<Triangle<T, 3>> &rs, int idYinset, Real tol) const
    {
    ...
    template <class T>
    inline YinSet<T> YinSet<T>::meet(const YinSet<T> &y2, const int usingRFB, Real tol) const
    {
