...
class Segment {
 public:
  enum locType { Inter = 0, ExtPoint = 1, Outer = 2 };

  enum intsType { None = 0, One = 1, Overlap = 2, EndPoint = 3 };
 ...
 public:
  Segment() = default;
  Segment(
      const Point<T, Dim>& stPoint,
      const Point<T, Dim>& fnPoint,
      std::vector<std::pair<int, int>> n = std::vector<std::pair<int, int>>())
      : neighbor(n) {
    endPoint[0] = stPoint;
    endPoint[1] = fnPoint;
  }
  Segment<T, Dim>(const Segment<T, Dim>& rhs) : neighbor(rhs.neighbor) {
  ...
  const std::vector<std::pair<int, int>>& neighborhood() const {
    return neighbor;
  }
  ...
  Segment<T, Dim - 1> project(int d = -1) const {
    ...
    auto stp = endPoint[0].project(d), fnp = endPoint[1].project(d);
    return Segment<T, Dim - 1>(stp, fnp);
  }

  // Estimate Point position with segment.
  locType containPoint(const Point<T, Dim>& p,
                       int mDim = -1,
                       Real tol = TOL) const {
    if (mDim == -1)
      mDim = majorDim();
    ...
    if (pointcmp.compare(endPoint[0], p) == 0 ||
        pointcmp.compare(endPoint[1], p) == 0) {
      return ExtPoint;
    }
    Vec<T, Dim> v1 = endPoint[1] - endPoint[0], v2 = p - endPoint[0];
    Real area = norm(cross(v1, v2)), bot = norm(v1);
    if (area / bot < tol) {
...
template <class T, int Dim>
inline typename Segment<T, 2>::intsType intersectSegLine(
    const Segment<T, Dim>& seg1,
    const Line<T, Dim>& l2,
    Real tol = TOL) {
  int mDim1 = seg1.majorDim(1), mDim2 = l2.majorDim(1), mDim;
  for (int d = 0; d < Dim; ++d) {
    if (d != mDim1 && d != mDim2)
      mDim = d;
  }
  auto proSeg1 = seg1.project(mDim), proL2 = l2.project(mDim);
...
template <class T>
inline typename Segment<T, 2>::intsType intersectSegLine(
    const Segment<T, 2>& seg1,
    Line<T, 2>& l2,
    std::vector<Point<T, 2>>& result,
    Real tol = TOL) {
  ...
  bool contain1 = l2.containPoint(p1, tol), contain2 = l2.containPoint(p2, tol);
...
template <class T, int Dim>
inline typename Segment<T, 2>::intsType intersectSegSeg(
    const Segment<T, Dim>& seg1,
    const Segment<T, Dim>& seg2,
    Real tol = TOL) {
  int mDim = cross(seg1[1] - seg1[0], seg2[1] - seg2[0]).majorDim(1);

  auto proSeg1 = seg1.project(mDim), proSeg2 = seg2.project(mDim);
...
inline typename Segment<T, 2>::intsType intersectSegSeg(
  ...
  Point<T, 2> p1 = seg1[0], p2 = seg1[1], p3 = seg2[0], p4 = seg2[1];
  ...
  if (std::abs(det / sc[0]) < tol) {  // parallel segments
...
template <class T, int Dim>
inline typename Segment<T, Dim>::intsType solveForOverlie(
    Point<T, Dim>& p1,
    Point<T, Dim>& p2,
    Point<T, Dim>& p3,
    Point<T, Dim>& p4,
