...
struct Triangulation {
  ...
  void tryEveryDiagonal(
    ...
    std::map<std::pair<int, int>, std::set<Segment<T, 3>, SegmentCompare>>
        mapallSeg;
    std::map<Point<T, 3>, std::set<Segment<T, 3>, SegmentCompare>, PointCompare>
        near(pCmp);
    ...
    while (!markEdge.empty()) {
      ...
      for (auto&& NextSeg : near.at(p1)) {
        ...
        if (segcmp.compare(NextSeg, itSeg0->first) == 0)
          continue;
        auto np = (pCmp.compare((NextSeg)[0], p1) == 0) ? ((NextSeg)[1])
                                                        : ((NextSeg)[0]);
      ...
      if (pCmp.compare(p1, (itSeg1->first)[0]) == 0) {
        assert((itSeg1->second).first == 1 &&
               "generatorTriangle itSeg1 wrong.");
        (itSeg1->second).first -= 1;
      } else {
        assert((itSeg1->second).second == 1 &&
               "generatorTriangle itSeg1 wrong.");
        (itSeg1->second).second -= 1;
      }
      if (pCmp.compare(p2, (itSeg2->first)[0]) == 0) {
        assert((itSeg2->second).first == 1 &&
               "generatorTriangle itSeg2 wrong.");
        (itSeg2->second).first -= 1;
      } else {
        assert((itSeg2->second).second == 1 &&
               "generatorTriangle itSeg2 wrong.");
