...
namespace YSB
{
  ...
  public:
    ...
    Tensor(const Box<Dim - 1> &_box, int ncomp)
        : Tensor(enlarge(_box, Box<1>(Vec<int, 1>(0), Vec<int, 1>(ncomp - 1))))
    {
    ...
    Tensor(const int i) : Tensor(Vec<int, 1>(i)) {}
  ...
  protected:
    ...
    static Vec<int, Dim> strideFromSize(const Vec<int, Dim> &sz);
  ...
  template <class T, int Dim>
  inline Tensor<T, Dim>::Tensor(const Tensor<T, Dim> &rhs) : owned(false), stride(rhs.stride), bx(rhs.bx)
  {
...
#pragma omp parallel for default(shared) schedule(static)
  ...
  template <class T, int Dim>
  inline Tensor<T, Dim> &Tensor<T, Dim>::operator=(Tensor<T, Dim> &&rhs) noexcept
  {
    swap(*this, rhs);
    return *this;
  }
  ...
  template <class T, int Dim>
  inline Tensor<T, Dim> &Tensor<T, Dim>::operator=(std::initializer_list<T> initlist)
  {
    int i = 0;
