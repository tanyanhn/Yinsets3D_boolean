...
TEST_CASE("Prepaste", "[pp1]") {
  ...
  Triangle<Real, 3> tri1(tripoints1), tri2(tripoints2), tri3(tripoints3),
      ...
      tri8(tripoints8), tri9(tripoints9);
  ...
  for (size_t i = 0; i < triB.size(); i++) {
    TriangulateB[i].push_back(i);
    for (int k = 0; k < 3; k++) {
      triB[i].ed(k).neighborhood().push_back(make_pair(2, i));
      for (size_t j = 0; j < triB.size(); j++) {
        if (j != i) {
          for (int h = 0; h < 3; h++) {
            if (norm(triB[j].ed(h)[0] - triB[i].ed(k)[0]) < TOL &&
                norm(triB[j].ed(h)[1] - triB[i].ed(k)[1]) < TOL)
              triB[i].ed(k).neighborhood().push_back(make_pair(2, j));
            if (norm(triB[j].ed(h)[0] - triB[i].ed(k)[1]) < TOL &&
                norm(triB[j].ed(h)[1] - triB[i].ed(k)[0]) < TOL)
              triB[i].ed(k).neighborhood().push_back(make_pair(2, j));
          }
        }
      }
