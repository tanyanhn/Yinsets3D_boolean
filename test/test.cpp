...
int main() {
  ...
  // for_each(vec.begin(), vec.end(), [](pair<int, int> p) { cout << " ( " <<
  ...
  **/
  ...
  // vector<vector<Triangle<Real, 3>>> a(row);
  // //row决定最里面层容器大小，vector<int>(column)决定外层容器的类型和大小 for
  // (int k = 0; k < row; k++)
